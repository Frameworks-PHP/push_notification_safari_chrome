...
function Message(agent, codec, opts) {
    ...
    if (opts.enc)
        this.encoding = opts.enc;

    // import custom variables
    for (var name in opts) {
        if (name === 'aps' || name === 'enc')
            continue;
    ...
    if (opts.aps) {
        // import badge/sound
        if (opts.aps.badge)
            this.badge(opts.aps.badge);
        if (opts.aps.sound)
            this.sound(opts.aps.sound);
        if (opts.aps['content-available'])
            this.contentAvailable(opts.aps['content-available']);
 ...
 * ### .append (key, value)
 *
 * Sets variables to be included in the `key` dictionary
 * for the `object` portion of the payload. If you wish to set
 * a singlular message, set the `aps` key. You may also set
 * any of the other values outlined in the APNS documentation
 * and the codecs will optimize the payload format for delivery.
 * ```js
 * // just set body
 * msg.append('aps', { 'alert' : 'Hello Universe' });
 *
 * Note: The value will be replaced if the key exists.
 *
 * @param {String} append key, string key or object of append settings
 * @param {Mixed} value (when first argument is a string|Object)
 * @returns {this} for chaining
 * @api public
 * @name append
 */

Message.prototype.apsAppend = function (key, value) {
    if (key === undefined)
        return this;
    var keys = Object.keys(this.aps);
    if (keys.length === 0) {
        this.aps[key] = value;
        return this;
    } else {
        for (var name in keys) {
            var opts = this.aps[keys[name]];
            if (opts !== undefined && 'object' === typeof opts) {
                this.aps[keys[name]] = util.findAppendArgs(opts, key, value);
            }
        }
        this.aps[key] = value;
    }
...
Message.prototype.serialize = function () {
    ...
    if (apsLength === 1 && this.aps.body && (typeof payload.aps.alert === 'string' || payload.aps.alert === undefined)) {
        payload.aps.alert = this.aps.body;
    } else if (apsLength > 0) {
        for (var name in this.aps) {
            if (payload.aps[name] === undefined) {
                payload.aps[name] = {};
                extend(payload.aps[name], this.aps[name]);
            }
        }
    }
    //console.log(this);
...
Message.prototype.send = function (cb) {
    cb = cb || function () {};
    if (!this._agent)
        return cb(new Error('Agent not associated with message.'));
