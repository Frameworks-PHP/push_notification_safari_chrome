...
//    var selectListSite = selectListSite;
    ...
    for(x in objectJson){
        ...
        var inputwebpushid = document.createElement('input');
        inputwebpushid.setAttribute("type", "hidden");
        inputwebpushid.id = objectJson[x]._id;
        inputwebpushid.setAttribute("value", objectJson[x].webpushid);
    }
    
    $( "#list-domain-send-noti" ).on( "change", function() {
        $("#list-webpushid-send-noti").val($("#"+this.value).val());
      });
}
...
function pushNotification() {
    ...
    fetch($("#list-domain-send-noti option:selected").text()+'/v1/push', {
        method: 'post',
        headers: {
            'Content-type': 'application/json'
        },
